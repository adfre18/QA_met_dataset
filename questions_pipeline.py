...
class QuestionGenerator(object):
    ...
    def generate_questions(self):
        ...
        with open(self.output_filename, 'w+', encoding='utf-8') as fw:
            for artist_name, context in tqdm(contexts.items()):
...
class QuestionsAnswering(object):
    ...
    def generate_answers(self):
        # generating answers
        with open(self.output_filename, 'w+', encoding='utf-8') as fw:
            for line_questions in tqdm(self.data_questions):
                    ...
                    # writing to file
                    print(f"{artist_name}", f"Question: {question}", f"Ref: {reference_answer.rstrip()}", f"Pred: {output.rstrip()}", f"Context: {context}",file=fw, sep="\t")
                    fw.flush()